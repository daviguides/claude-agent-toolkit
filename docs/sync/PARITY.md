...
**Total: 225** — ported: 190, justified_gap: 35, not_ported: 0, partial: 0
...
| `claude_agent_sdk.list_sessions` | function | `session_management::disk::list_sessions` | ported | true | Direct-local-disk family: scans every matching ~/.claude/projects/<sanitized>/ directory (or a specific one, resolved via canonicalize+sanitize+git-worktree fallback), lite-reads each session's head/tail (64KiB each), dedupes by session id keeping the freshest mtime, sorts descending, paginates. |
| `claude_agent_sdk.get_session_info` | function | `session_management::disk::get_session_info` | ported | true | Direct-local-disk family: resolves the project directory, lite-reads the session's head/tail, parses summary metadata (title precedence, first-prompt extraction, tag scan, created_at) without loading the full transcript. |
| `claude_agent_sdk.get_session_messages` | function | `session_management::disk::get_session_messages` | ported | true | Direct-local-disk family: full-file read, parses transcript entries, rebuilds the multi-leaf/terminal-hunting conversation chain (shared with the store family via session_management::store::build_conversation_chain), filters to visible messages, paginates. |
| `claude_agent_sdk.list_subagents` | function | `session_management::disk::list_subagents` | ported | true | Direct-local-disk family: recursively collects agent-*.jsonl files under the session's subagents/ directory (supports nested subagents/workflows/<run>/ layouts), extracts agent ids. |
| `claude_agent_sdk.get_subagent_messages` | function | `session_management::disk::get_subagent_messages` | ported | true | Direct-local-disk family: locates the matching agent-<id>.jsonl file (recursive search), builds the linear subagent chain (session_management::store::build_subagent_chain), paginates. |
| `claude_agent_sdk.SDKSessionInfo` | field | `session_management::SDKSessionInfo` | ported | true | Pure data type shared by both families' info/list functions. |
| `claude_agent_sdk.SessionMessage` | field | `session_management::SessionMessage` | ported | true | Pure data type shared by both families' message-chain functions. |
| `claude_agent_sdk.InMemorySessionStore` | class | `session_management::store::InMemorySessionStore` | ported | true | Reference SessionStore implementation backed by in-process HashMaps (entries/mtimes/summaries, monotonic mtime clock); exposes get_entries/size/clear test helpers matching upstream's own conformance-test surface. |
| `claude_agent_sdk.fold_session_summary` | function | `session_management::summary::fold_session_summary` | ported | true | Pure incremental-fold function; exposed publicly so a custom SessionStore adapter can maintain its own list_session_summaries sidecar the way InMemorySessionStore does internally. |
| `claude_agent_sdk.project_key_for_directory` | function | `session_management::paths::project_key_for_directory` | ported | true | Canonicalize (realpath + NFC) then sanitize (non-alphanumeric -> '-', hash-suffixed truncation past 200 chars via the CLI's own 32-bit rolling hash, base36-encoded). |
| `claude_agent_sdk.import_session_to_store` | function | `session_management::disk::import_session_to_store` | ported | true | Appends caller-supplied entries into a SessionStore-backed session, mirroring what a live session mirror would have written. |
| `claude_agent_sdk.list_sessions_from_store` | function | `session_management::store::list_sessions_from_store` | ported | true | SessionStore-backed family: prefers list_session_summaries with gap-fill via load() for stale/missing sidecars (list_sessions cross-check); falls back to bounded-concurrency (16) full loads when list_session_summaries isn't implemented. Sorted desc, paginated (limit=Some(0) means unlimited, matching upstream's truthy check). |
| `claude_agent_sdk.get_session_info_from_store` | function | `session_management::store::get_session_info_from_store` | ported | true | SessionStore-backed family: load() + fold_session_summary + summary-to-info conversion. |
| `claude_agent_sdk.get_session_messages_from_store` | function | `session_management::store::get_session_messages_from_store` | ported | true | SessionStore-backed family: load() + shared conversation-chain builder + visibility filter + pagination. |
| `claude_agent_sdk.list_subagents_from_store` | function | `session_management::store::list_subagents_from_store` | ported | true | SessionStore-backed family: list_subkeys() filtered to subagents/ prefix, agent id extraction, order-preserving dedup. |
| `claude_agent_sdk.get_subagent_messages_from_store` | function | `session_management::store::get_subagent_messages_from_store` | ported | true | SessionStore-backed family: resolves the exact subpath via list_subkeys (handles nested subagents/workflows/<run>/agent-<id>), falls back to the flat subagents/agent-{id} path when list_subkeys isn't implemented; drops synthetic agent_metadata entries before chain-building. |
| `claude_agent_sdk.rename_session` | function | `session_management::disk::rename_session` | ported | true | Direct-local-disk family: appends a 3-key custom-title entry (no uuid/timestamp, distinct wire shape from the _via_store sibling) directly to the session's .jsonl file, creating the project directory on first write. |
| `claude_agent_sdk.tag_session` | function | `session_management::disk::tag_session` | ported | true | Direct-local-disk family: Unicode-sanitizes the tag (NFKC + Cf/Co/Cn category strip + explicit zero-width/BOM/directional-mark ranges, iterated to a fixed point), appends a 3-key tag entry. |
| `claude_agent_sdk.delete_session` | function | `session_management::disk::delete_session` | ported | true | Direct-local-disk family: removes the session's .jsonl file and its subagents/ directory, best-effort (missing files are not an error). |
| `claude_agent_sdk.fork_session` | function | `session_management::disk::fork_session` | ported | true | Direct-local-disk family: shares the uuid-remapping/parent-chain-rebuilding transform (session_management::disk::build_fork_lines) with fork_session_via_store; writes via create_new (O_EXCL semantics) with 0o600 permissions on unix. |
| `claude_agent_sdk.ForkSessionResult` | field | `session_management::ForkSessionResult` | ported | true | Pure data type: the forked session's id. |
| `claude_agent_sdk.rename_session_via_store` | function | `session_management::store::rename_session_via_store` | ported | true | SessionStore-backed family: appends a 5-key custom-title entry (uuid + timestamp included, distinct wire shape from the disk sibling). |
| `claude_agent_sdk.tag_session_via_store` | function | `session_management::store::tag_session_via_store` | ported | true | SessionStore-backed family: same Unicode sanitization as the disk sibling, appends a 5-key tag entry via store.append. |
| `claude_agent_sdk.delete_session_via_store` | function | `session_management::store::delete_session_via_store` | ported | true | SessionStore-backed family: a no-op (not an error) when the store doesn't implement delete — correct for write-once/append-only backends. |
| `claude_agent_sdk.fork_session_via_store` | function | `session_management::store::fork_session_via_store` | ported | true | SessionStore-backed family: shares build_fork_lines with fork_session; appends the rebuilt lines to a freshly-keyed session via store.append. |
